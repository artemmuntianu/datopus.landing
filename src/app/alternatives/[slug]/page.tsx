...
export async function generateMetadata(props: Props): Promise<Metadata> {
    ...
    return {
        ...
        openGraph: {
            ...
            description,
            type: 'article',
            url: `${DATA.url}/alternatives/${post.slug}`,
            images: [
                {
                    url: ogImage,
                }
...
export default async function CompareUsPost(props: Props) {
    const { slug } = await props.params;
    ...
    return (
        <main className='flex flex-col mt-[3.5rem]'>
            <section className='blog-article min-h-[100dvh] text-lg mx-auto pb-8 px-4 md:max-w-[55rem]'>
                ...
                <RecommendedPosts post={post} allPosts={await getPosts(DATA.dirRelativePath)} baseUrl={`${DATA.url}/alternatives`} />
            </section>
